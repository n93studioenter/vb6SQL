...
'====================================================================================================
Public Function ConvertCStrToCast(ByVal strSQL As String) As String
    ...
    result = strSQL
    pos = InStr(1, result, "CStr(", vbTextCompare)
    
    Do While pos > 0
        ' Tìm d?u ngo?c dóng tuong ?ng
        depth = 1
        i = pos + 5  ' B? qua "CStr("
        
        Do While i <= Len(result) And depth > 0
        ...
        If i <= Len(result) Then
            ' L?y n?i dung bên trong CStr(...)
            innerExpr = Mid(result, pos + 5, i - pos - 5)
            
            ' Thay th? CStr(...) b?ng CAST(... AS NVARCHAR)
            result = Left(result, pos - 1) & _
                     "CAST(" & innerExpr & " AS NVARCHAR)" & _
                     Mid(result, i + 1)
        End If
        
        ' Tìm CStr ti?p theo
        pos = InStr(pos + 10, result, "CStr(", vbTextCompare)
    Loop
    
    ConvertCStrToCast = result
End Function
Public Sub SetSQL(qname As String, SQL As String)
    ...
    sqlServer = ConvertCStrToCast(sqlServer)
...
d:
    ...
    ' Kh?i t?o ph?n FROM và WHERE m?t l?n
    Dim SQL_From As String
    Dim SQL_Where As String
    SQL_From = "FROM SoDuKhachHang INNER JOIN KhachHang ON SoDuKhachHang.MaKhachHang = KhachHang.MaSo"
    SQL_Where = "WHERE KhachHang.MaNT <> 0"

    For i = 1 To 12
        ' T?o câu l?nh hoàn ch?nh trong vòng l?p
        SQL = "UPDATE SoDuKhachHang " & _
              "SET DuNT_" & CStr(i) & " = ABS(DuNT_" & CStr(i - 1) & _
            " + IIF(DuNo_" & CStr(i - 1) & " - DuCo_" & CStr(i - 1) & " >= 0, " & _
              "No_" & CStr(i) & "_NT - Co_" & CStr(i) & "_NT, " & _
              "Co_" & CStr(i) & "_NT - No_" & CStr(i) & "_NT)) " & _
              SQL_From & " " & SQL_Where
